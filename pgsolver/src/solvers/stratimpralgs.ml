...
let lower_bound_construction_round_robin_compare_exp game (v,w) (v',w') =
	...
	in
	let g = function		
		('d','e') -> 1   (* complex ordering ... *)
	|	('u', _ ) -> 1	 (* complex ordering ... *)
	|	('b','u') -> 1   (* complex ordering ... *)
	|	('a', _ ) -> 2   (* ordering irrelevant *)
	|	('d','p') -> 3   (* ordering irrelevant *)
	|	('b','e') -> 4   (* ordering irrelevant *)
	|	('w', _ ) -> 5   (* top-down ordering *)
	|	_ -> -1
	in
	let h = function
		('d',_) -> 2
...
let strategy_improvement_optimize_roundrobin_policy_lower_bound game =
	let lookup_node s =
        ...
        if !i < n then Some !i else None
    in
    let is_exp = lookup_node "d0" != None in
	let cmp = if is_exp
